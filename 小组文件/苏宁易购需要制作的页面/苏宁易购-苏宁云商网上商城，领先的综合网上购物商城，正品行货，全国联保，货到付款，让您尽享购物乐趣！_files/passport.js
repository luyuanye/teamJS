    ...
    if (newHash.length > 1) {
        ...
        switch (params[0]) {
            ...
            case 'loginSuccess':
                ...
                break;
            case 'loginRedirect':
                location.href = decodeURIComponent(params[1]);
                break;
            default:
                break;
        }
...
function closeContainer() {
    ...
    $('html').removeClass('login-noscroll2');
    $('body').removeClass('login-noscroll');
    if(wapPopSt != 1){
    	$('body').scrollTop(wapPopSt);
    	wapPopSt = 1;
    }
...
function ensureLoginWap(callback, config){
	if (typeof config == 'undefined') {
		if (typeof passport_config != 'undefined') {
			config = passport_config;
		} else {
    ...
    $("body").append("<div id=\"modalOverlay\" style=\"position: absolute; top: 0; left: 0; z-index: 998;\"></div><div id=\"modalContainer\"></div>");
//    $("#modalOverlay").hide();
    $("#modalOverlay").height("100%").width("100%");
    $("#modalOverlay").html("<iframe id=\"iframeLogin\"  src=\"" + src
        + "\" width=\"100%\" height=\"100%\" scrolling=\"no\" frameborder=\"0\"></iframe>");
    if(!isAppendLoginStyle){
    	$("head").append("<style>.login-noscroll{overflow:hidden;height:100%} .login-noscroll2{overflow:scroll;height:100%}</style>");
    	isAppendLoginStyle = true;
    }
    wapPopSt = $('body').scrollTop();
    $("html").addClass("login-noscroll2");
    $("body").addClass("login-noscroll");
    if (typeof intervalVar != 'undefined') {
    	clearInterval(intervalVar);
    }
...
function ensureReg(callback, config) {
	probeAuthStatus(function(){if(callback!=null){callback()}},
			function(){popupRegContainer(callback, config)},config);
}

function popupRegContainer(callback, config) {
	...
	if(!config.hasOwnProperty("regDomain")){
		alert('You must define passport_config.regDomain');
		return;
	}
	loginCallbackStack.push(callback);
    currentLocation = window.location.href;
    var src = config.regDomain + "/popreg.do?" + "topLocation=" + encodeURIComponent(currentLocation.split('#')[0]);
    ...
    $("#modalOverlay").hide();
    $("#modalOverlay").css("height",$(document).height()+"px");
    $("#modalContainer").html("<iframe id=\"iframeLogin\" style=\"position: fixed ;\" width=\"412\" height=\"1\" src=\"" + src
        + "\" frameborder=\"0\" scrolling=\"no\" allowtransparency=\"yes\"></iframe>");
    if (typeof intervalVar != 'undefined') {
    	clearInterval(intervalVar);
    }
